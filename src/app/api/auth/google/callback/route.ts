...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!GOOGLE_REDIRECT_URI) {
      console.error('[Google OAuth] Missing GOOGLE_REDIRECT_URI');
    ...
    if (!backendResponse.ok) {
      ...
      console.error('[Google OAuth] Backend error:', errorMessage);

      // Handle 409 conflict (email already registered with password account)
    ...
    if (!token) {
      console.error('[Google OAuth] No token received from backend');
      return NextResponse.redirect(new URL('/login?error=oauth_no_token', request.url));
    }

    // Set httpOnly cookie (same settings as Kakao / email login)
